...
mod tests {
    ...
    fn parse_expression_basic() {
        let expr = parse_expression("concat(first, \" \" , last)").unwrap();
        match expr {
            Expr::Function { name, .. } => assert_eq!(name, "concat"),
            _ => panic!("expected function"),
        }
    }

    #[test]
    fn transform_plan_replace() {
        let input = TransformConfigInput {
            mode: TransformMode::Replace,
            fields: vec![FieldMapInput {
                target_field_name: "id".to_string(),
                origin_field_name: Some("user_id".to_string()),
                required: Some(true),
                default_value: None,
                coerce: None,
                compute: None,
        ...
        let plan = TransformPlan::compile(input).unwrap();
        let mut record = Map::new();
        record.insert("user_id".to_string(), Value::Number(Number::from(42)));
        let output = plan.apply_to_record(&record).unwrap().unwrap();
        assert_eq!(output.get("id").unwrap(), &Value::Number(Number::from(42)));
    }

    #[test]
    fn transform_coerce_timestamp() {
        let input = TransformConfigInput {
            mode: TransformMode::Replace,
            fields: vec![FieldMapInput {
                target_field_name: "created".to_string(),
                origin_field_name: Some("created".to_string()),
                required: Some(true),
                default_value: None,
                coerce: Some(CoerceSpec::TimestampMs {
                    format: Some(TimestampFormat::UnixS),
                }),
                compute: None,
            }],
            on_missing_field: None,
        ...
        let plan = TransformPlan::compile(input).unwrap();
        let mut record = Map::new();
        record.insert("created".to_string(), Value::Number(Number::from(2)));
        let output = plan.apply_to_record(&record).unwrap().unwrap();
        assert_eq!(output.get("created").unwrap(), &Value::Number(Number::from(2000)));
    }
    ...
    fn transform_missing_required_errors() {
        let input = TransformConfigInput {
            ...
            fields: vec![FieldMapInput {
                ...
                origin_field_name: Some("email".to_string()),
                required: Some(true),
                default_value: None,
                coerce: None,
                compute: None,
            ...
            on_missing_field: None,
            on_missing_required: Some(MissingRequiredPolicy::Error),
            on_coerce_error: None,
        };

        let plan = TransformPlan::compile(input).unwrap();
        let record = Map::new();
        let err = plan.apply_to_record(&record).unwrap_err();
        assert!(err.to_string().contains("Missing required field"));
    }

    #[test]
    fn transform_missing_field_null_policy() {
        let input = TransformConfigInput {
            mode: TransformMode::Replace,
            fields: vec![FieldMapInput {
                target_field_name: "country".to_string(),
                origin_field_name: Some("country".to_string()),
                required: Some(false),
                default_value: None,
                coerce: None,
                compute: None,
            }],
            on_missing_field: Some(MissingFieldPolicy::Null),
            on_missing_required: None,
            on_coerce_error: None,
        };
        ...
        let record = Map::new();
        let output = plan.apply_to_record(&record).unwrap().unwrap();
        assert_eq!(output.get("country").unwrap(), &Value::Null);
    }

    #[test]
    fn transform_coerce_error_drop_record() {
        let input = TransformConfigInput {
            mode: TransformMode::Replace,
            fields: vec![FieldMapInput {
                target_field_name: "count".to_string(),
                origin_field_name: Some("count".to_string()),
                required: Some(false),
                default_value: None,
                coerce: Some(CoerceSpec::I64),
                compute: None,
            }],
            on_missing_field: None,
            on_missing_required: None,
            on_coerce_error: Some(CoerceErrorPolicy::DropRecord),
        };

        let plan = TransformPlan::compile(input).unwrap();
        let mut record = Map::new();
        record.insert("count".to_string(), Value::String("oops".to_string()));
        let output = plan.apply_to_record(&record).unwrap();
        assert!(output.is_none());
    }

    #[test]
    fn transform_compute_concat_and_upper() {
        let input = TransformConfigInput {
            mode: TransformMode::Replace,
            fields: vec![
                FieldMapInput {
                    target_field_name: "full".to_string(),
                    origin_field_name: None,
                    required: None,
                    default_value: None,
                    coerce: None,
                    compute: Some("concat(first, \" \", last)".to_string()),
                },
                FieldMapInput {
                    target_field_name: "shout".to_string(),
                    origin_field_name: None,
                    required: None,
                    default_value: None,
                    coerce: None,
                    compute: Some("upper(first)".to_string()),
                },
            ],
            on_missing_field: None,
            on_missing_required: None,
        ...
        let plan = TransformPlan::compile(input).unwrap();
        let mut record = Map::new();
        record.insert("first".to_string(), Value::String("Jane".to_string()));
        record.insert("last".to_string(), Value::String("Doe".to_string()));
        let output = plan.apply_to_record(&record).unwrap().unwrap();
        assert_eq!(output.get("full").unwrap(), &Value::String("Jane Doe".to_string()));
        assert_eq!(output.get("shout").unwrap(), &Value::String("JANE".to_string()));
    }

    #[test]
    fn engine_streams_records_across_chunks() {
        let input = TransformConfigInput {
            mode: TransformMode::Replace,
            fields: vec![FieldMapInput {
                target_field_name: "id".to_string(),
                origin_field_name: Some("id".to_string()),
                required: Some(true),
                default_value: None,
                coerce: Some(CoerceSpec::I64),
                compute: None,
        ...
        let first = engine.push(br#"{"id":"1"}"#).unwrap();
        assert!(first.output.is_empty());
        let second = engine.push(b"\n{\"id\":\"2\"}\n").unwrap();
        assert_eq!(second.records, 2);
        let output = String::from_utf8(second.output).unwrap();
